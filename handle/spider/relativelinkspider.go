...
// var extlinks []extractedlink	//	@Consider just []string
var extlinks []string
...
type extractedlink struct {	//	@TODO	add src
	contents		string
}
	...
	if strings.Contains(r, "Target Responds in HTTPS - Cannot Follow through with HTTP Methods Checking") {	//	or
		...
		doc.Find("*[href]").Each(func(i int, href *goquery.Selection) {
			...
			if oklink {
				...
				// if !h.isstrinlinks(link) && h.isvalidrellink(link) {
				// 	if strings.HasPrefix(link,"/") || strings.HasPrefix(link,"#") {
				// 		extlinks = append(extlinks, link)
				// 	} else {	//	@TODO	Consider symbol checking
				// 		extlinks = append(extlinks, "/" + link)
				// 	}
				// }
			}
...
// func (h *RelativeLinkSpider) uniquenesscheck(tocheck []string) {					//	tocheck []extractedlink
